...
// hedgetrimmers ---------------------------------
app.get("/api/hedgetrimmers/:id", (req, res) => {
    const productID = req.params.id;
    res.send(data.hedgetrimmers.find(x => x._id === productID));
    if (product) {
        res.send(product);
    } else {
        res.status(404).send({
...
app.get("/api/hedgetrimmers", (req, res) => {
    res.send(data.hedgetrimmers)
});

// blowers --------------------------------------------------------
app.get("/api/blowers/:id", (req, res) => {
    const productID = req.params.id;
    res.send(data.blowers.find(x => x._id === productID));
    if (product) {
        res.send(product);
    } else {
        res.status(404).send({
...
app.get("/api/blowers", (req, res) => {
    res.send(data.blowers)
});
